...
static void init() {
#if defined(NO_SERVER) || defined(NO_PBX) || defined(NO_TU)
  cr_assert_fail("Not all the modules were implemented\n");
    ...
    if((server_pid = fork()) == 0) {
        struct rlimit limit = { .rlim_cur=120, .rlim_max=120 };
...
Test(SUITE, TEST_NAME, .init = init, .fini = killall, .timeout = 120)
{
	...
	if(pids[i] == 0) { // Child
	    ...
	    if(execl("util/tester", "tester", "-p", SERVER_PORT_STR, "-l", TEST_LENGTH,
		     "-x", "0", "-y", "15", "-d", "0", NULL) < 0) {
