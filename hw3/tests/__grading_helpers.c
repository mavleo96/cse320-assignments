...
// TODO: The meaning of the value returned here is not the same as
// the value passed to sf_set_slop().
size_t slop() {
    if(!((size_t)sf_mem_start() & 0x1f))
	return 0x18;
    else
	return 0x8;
}

// Use this in initialization to get a heap that is 16-byte aligned,
// but not 32-byte aligned.
void set_16_byte_alignment() {
    sf_set_slop(16);
}
...
void _assert_block_is_valid(sf_block *bp)
{
    ...
    size_t block_size = bp->header & ~0x1f;
    size_t aligned_sz = ((block_size + 31) >> 5) << 5;
    cr_assert(block_size == aligned_sz, "Block %p is not properly aligned", &bp->header);

    cr_assert(block_size >= 32 && block_size <= PAGE_SZ * 100 && (block_size & 0x1f) == 0,
    ...
    cr_assert(nbp == ((sf_block *)(sf_mem_end() - 8)) ||
	                (((nbp->header & 0x8) != 0) == ((bp->header & 0x10) != 0)),
	     "Prev allocated bit is not correctly set for %p. Should be: %d",
         &(nbp)->header, bp->header & 0x8);

    // other issues to check
    cr_assert((bp->header & 0x10) || ((*(sf_header *)((char *)nbp - 8)) == bp->header),
...
void _assert_heap_is_valid(void)
{
    ...
    for(bp = ((sf_block *)(sf_mem_start() + slop()));
	limit-- && bp < (sf_block *)((size_t)sf_mem_end() - (32 - slop()));
	bp = (sf_block *)((char *)(bp) + (bp->header & ~0x1f)))
	_assert_block_is_valid(bp);
    cr_assert(bp == (sf_mem_end() - (32 - slop())), "Could not traverse entire heap");
...
{
    cr_assert(((bp->header & 0x10) != 0) == alloc,
	      "Block %p has wrong allocation status (got %d, expected %d)",
	      &bp->header, (bp->header & 0x10) != 0, alloc);

    cr_assert((bp->header & ~0x1f) == b_size,
	      "Block %p has wrong block_size (got %lu, expected %lu)",
	      &bp->header, bp->header & ~0x1f, b_size);
}
