...
-(i.e. 7 for this assignment), which contains blocks whose size is greater than `21M`.
The last list (at index `NUM_FREE_LISTS-1`; *i.e.* 8) is only used to contain
...
```c
    ...
    |                                  (5 LSB's implicitly 0)    | (0/1)  |  (0/1)  |   (0)   |
...
- The `block_size` field gives the number of bytes for the **entire** block (including header/footer,
  ...
  except that two of the five least-significant bits of the block size, which would normally always
...
- The `prev_alloc` (bit 3, mask 0x8) is also a boolean. It is 1 if the **immediately preceding** block
...
> Any functions other than `sf_malloc`, `sf_free`, `sf_realloc`, `sf_memalign`
...
Although the x86-64 architecture does not support basic values larger than 16 bytes,
we are requiring that your allocator satisfy the slightly larger 32-byte alignment simply
in order vary the assignment parameters.
...
> structure definitions to easily access the various fields.  You will sometimes need
> to do actual address calculations; these should be carried out by casting pointers to
> type `char *` before doing the calculations and casting them back to the required
> pointer type afterwards.
...
> :scream:  Note that there is no `footer` field in the `sf_block` struture.
> This is because the footer of a block will occur an unknown distance from the header,
> due to the variable size of the payload area.  In order to obtain the footer of a
> block, you will have to take the pointer to the beginning of the block and use the
> block size to computer the pointer to the start of the next block.  The footer
> field occurs one memory row (8 bytes) before the start of the next block.
...
In order for the first heap block to have its payload area be 32-byte aligned,
it is necessary for the header of the first heap block to be stored at an offset of
either 24 bytes or 8 bytes from the start of the heap (depending on whether
the start of the heap is already 32-byte aligned or just 16-byte aligned).
Thus, the first 24 or 8 bytes of the heap will be unused.
The last 8 bytes of the heap will also be unused.  These 8 bytes are reserved
to become the header of the next block when the heap is extended using
`sf_mem_grow`.  The payload area of this block will thus be aligned on a 32-byte
...
```c
    ...
    +------------------------------------------------------------+--------+---------+---------+ <- header
    |                                       block_size           | alloc  |prv alloc|  unused |
    |                                  (5 LSB's implicitly 0)    |  (1)   |  (0/1)  |   (0)   | prologue
    |                                        (1 row)             | 1 bit  |  1 bit  |  3 bits |
    +------------------------------------------------------------+--------+---------+---------+ <- (32-byte aligned)
    |                                                                                         |
    |                                        Padding                                          |
    ...
    +------------------------------------------------------------+--------+---------+---------+ <- header
    |                                       block_size           | alloc  |prv alloc|  unused |
    |                                  (5 LSB's implicitly 0)    |  (0)   |  (0/1)  |   (0)   | first block
    |                                        (1 row)             | 1 bit  |  1 bit  |  3 bits |
    +------------------------------------------------------------+--------+---------+---------+ <- (32-byte aligned)
    |                                                                                         |
    |                                   Payload and Padding                                   |
