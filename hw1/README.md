...
- The arguments (`-h` `-s` `-d`) are positional arguments which, if present,
must appear before any option arguments (`-c` or '-p'). The option arguments
may come in any order after the positional ones.
...
In `transplant.c`, you will find the following function prototype (function
...
The `-p` parameter is used to specify a pathname to be used either as a source
directory to be serialized or as a target directory for deserialization.
...
The file `global.h` contains function prototypes for functions that you **must**
...
The program will read serialized data byte-by-byte from the standard input
and write serialized data byte-by-byte to the standard output.
You should use `getchar()` or `fgetc()` to read an input byte and
`putchar()` or `fputc()` to write an output byte.  Once all the output has been
...
- To do file I/O, you should use the C standard I/O library.  Specifically,
  ...
  These functions are documented in Section 3 of the Linux manual pages.  For example,
